...
pub fn subcommand<'a>() -> App<'a> {
    App::new("validate")
        .about("Validates a docubus.json file. Downloads the schema.json file from GitHub if it's not found.")
        .version("0.1.0")
        .arg(
            Arg::with_name("file")
                .index(1)
...
pub async fn run(app: &ArgMatches, store: &mut Store) {
    if let Some(ref verify_command) = app.subcommand_matches("validate") {
        let file = verify_command.value_of("file").unwrap_or("docubus.json");
